...
func GeneratePolicy(conf OssConf) (policyToken PolicyToken, err error) {
	now := time.Now().Unix()
	...
	callbackParam.CallbackBody = "name=${object}&size=${size}&mimeType=${mimeType}&height=${imageInfo.height}&width=${imageInfo.width}&userId=${x:userId}&orgId=${x:orgId}"
	...
	policyToken.Xvar = make(map[string]interface{})
	return policyToken, nil
}

func HandlerCallBack(r *http.Request) (err error) {
